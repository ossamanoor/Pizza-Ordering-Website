var cart = {
  ...
  init : () => {
    ...
    for (let id in products) {
      ...
      item.querySelector(".p-price").textContent = "PKR " + p.price.toFixed(2);
  ...
  list : () => {
    ...
    // (D3) CART IS NOT EMPTY - LIST ITEMS
      ...
      item.id = "c-total";
      item.innerHTML ="TOTAL: PKR  " + total;
      cart.hItems.appendChild(item);
  ...
  change : (pid, qty) => {
    ...
    // (F2) UPDATE TOTAL ONLY
      ...
      for (let id in cart.items) {
        ...
        document.getElementById("c-total").innerHTML ="TOTAL: PKR  " + total;
      }
    }
  },
  ...
  checkout : () => {
    ...
    alert("Your Meals is On The Way wait for 30 mint");
